...
<body>
    <div class="register-container">
        ...
        @php
            $activeProviders = \App\Models\OAuthProvider::getActiveProviders();
        @endphp
        @if($activeProviders->count() > 0)
        <div style="margin-bottom: 30px;">
            <p style="color: #666; margin-bottom: 15px;">O regístrate con:</p>
            <div style="display: flex; gap: 10px; flex-direction: column;">
                @foreach($activeProviders as $oauthProvider)
                    @if($oauthProvider->provider === 'google')
                        <a href="{{ route('social.redirect', 'google') }}" 
                           style="display: flex; align-items: center; justify-content: center; padding: 10px; background-color: #db4437; color: white; text-decoration: none; border-radius: 4px; transition: background-color 0.3s;">
                            ...
                            Continuar con {{ $oauthProvider->name }}
                        </a>
                    ...
                    @else
                        <a href="{{ route('social.redirect', $oauthProvider->provider) }}" 
                           style="display: flex; align-items: center; justify-content: center; padding: 10px; background-color: #6c757d; color: white; text-decoration: none; border-radius: 4px; transition: background-color 0.3s;">
                            <i class="fab fa-{{ $oauthProvider->provider }}" style="margin-right: 10px;"></i>
                            Continuar con {{ $oauthProvider->name }}
                        </a>
                    @endif
                @endforeach
