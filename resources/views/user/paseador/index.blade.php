                ...
                <div class="card">
                    <div class="card-header">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <span id="card_title">
                                <i class="fas fa-walking"></i> {{ __('Mi Perfil de Paseador') }}
                            </span>
                        </div>
                    </div>
                        ...
                        @foreach ($users as $user)
                            <div class="row">
                                ...
                                <div class="col-md-8">
                                    ...
                                    <div class="row mt-4">
                                        <div class="col-12">
                                            <h5>Información de Paseador</h5>
                                            <ul class="list-unstyled">
                                                <li><strong>Barrios Asignados:</strong>
                                                    @if($user->barrios->count() > 0)
                                                        @foreach($user->barrios as $barrio)
                                                            <span class="badge bg-secondary">{{ $barrio->nombre }}</span>
                                                        @endforeach
                                                    @else
                                                        <span class="text-muted">Sin barrios asignados</span>
                                                    @endif
                                                </li>
                                                <li><strong>Mascotas Asignadas:</strong> {{ $user->mascotas_count ?? 0 }}</li>
                                                <li><strong>Calificación Promedio:</strong>
                                                    @if($user->calificacion_promedio)
                                                        @for($i = 1; $i <= 5; $i++)
                                                            <i class="fas fa-star {{ $i <= $user->calificacion_promedio ? 'text-warning' : 'text-muted' }}"></i>
                                                        @endfor
                                                        ({{ number_format($user->calificacion_promedio, 1) }})
                                                    @else
